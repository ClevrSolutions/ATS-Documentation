---
title: "ATS 1 How-To's"
category: "How-To's"
---

These are the documents in this section:

* [How to Get Started](getting-started)
* [How to Install the ATS Helper and ATS Recorder](install-ats-helper-recorder)
* [How to Create a Test Case](create-a-test-case)
* [How to Create a Test Suite](create-a-test-suite)
* [How to Create Custom Actions](create-custom-actions)
    
    * [General](custom-action-general)
        
        * [Definitions](custom-action-definitions)
        * [Prerequisites for How-To's](custom-action-prerequisites)
        * [Structure for How-To's](custom-action-howto-structure)
        * [Helpful Resources](custom-action-helpful-resources)
    * [Custom Action Basics](custom-action-basics)
    * [Create Unsupported Widget Actions](create-unsupported-widget-actions)
        
        * [CAB.02 - Switch](cab-02-switch)
        * [CAB.03 - TextBox](cab-03-textbox)
        * [CAB.05 - Reference Selector](cab-05-reference-selector)
        * [CAB.07 - Radio Buttons](cab-07-radiobuttons)
        * [CAB.10 - AutoComplete](cab-10-autocomplete)
    * [Create Search Context Actions](create-search-context-actions)
        * [CAB.11 - Find Item/Row by Unique Text Value](cab-11-find-itemrow)

* [Upload a file in your app using ATS](upload-file-using-ats-1)