...
It is now possible to see a video recording for test cases executed on Browserstack or Saucelabs in ATS. When looking at the result log for a test case execution you can see the recording under the new **Recording** tab. Please note that we do not store these recordings, we only link to the recorded videos from the respective selenium providers. Important note: To be able to see a recording from Saucelabs in ATS please set the following capability in your selenium hub configuration ```recordMp4:  true```.
